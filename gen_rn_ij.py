...
adj_with_sens_np[2:2+len(lnc_dmap_np), :]
adj_with_sens_np[2+len(lnc_dmap_np):2+len(lnc_dmap_np)+len(mi_dmap_np), :]
...
for i in range(5):
    ...
    pos_train_ij = pos_train_ij[(pos_train_ij[:, 0] > n_circ+n_lnc) & (pos_train_ij[:, 0] < n_circ + n_lnc + n_mi)]
    pos_test_ij = pos_test_ij[(pos_test_ij[:, 0] > n_circ+n_lnc) & (pos_test_ij[:, 0] < n_circ + n_lnc + n_mi)]
    unlabelled_train_ij = unlabelled_train_ij[(unlabelled_train_ij[:, 0] > n_circ+n_lnc) & (unlabelled_train_ij[:, 0] < n_circ + n_lnc + n_mi)]
    unlabelled_test_ij = unlabelled_test_ij[(unlabelled_test_ij[:, 0] > n_circ+n_lnc) & (unlabelled_test_ij[:, 0] < n_circ + n_lnc + n_mi)]

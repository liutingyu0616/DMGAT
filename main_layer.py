...
for n_gcn in range(1, 6):
    for n_gat in range(1, 6):
        logger = Logger(5)
        for i in range(5):
            print(f"fold {i}")
            ...
            r_gcn_list = [GCN(
                ...
                out_dim=gcn_out_dim,
                adj=rna_sim
            ).to(device) for _ in range(n_gcn)]
            ...
            gat_list = [GAT(
                ...
                alpha=0.1,
                nheads=n_heads
            ).to(device) for _ in range(n_gat)]
        ...
        logger.save(f"mesh_search\DMGAT_{n_gcn}_gcn_{n_gat}_gat")

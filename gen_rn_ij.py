...
adj_with_sens_np[:len(lnc_dmap_np), :]
adj_with_sens_np[len(lnc_dmap_np):len(lnc_dmap_np)+len(mi_dmap_np), :]
...
for i in range(5):
    print(f"fold {i}")
    pos_train_ij_all = pos_train_ij_list[i]
    pos_test_ij = pos_test_ij_list[i]
    unlabelled_train_ij = unlabelled_train_ij_list[i]
    unlabelled_test_ij = unlabelled_test_ij_list[i]

    pos_train_ij = pos_train_ij_all[(pos_train_ij_all[:, 0] > n_lnc) & (pos_train_ij_all[:, 0] < n_lnc + n_mi)]
    ...
    rn_indices = sorted_indices[:len(pos_train_ij_all)-len(sens_train_ij)]
